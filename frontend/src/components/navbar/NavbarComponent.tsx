...
const NavbarComponent = () => {
  ...
  const cartSlicer = useSelector((store: any) => store.cartSlicer);
  const navigate = useNavigate();
  const [inputSearch, setInputSearch] = useState("");
  const { t } = useTranslation();
  let subtotal = 0;

  cartSlicer.cart.map((el: Cart) => {
  ...
  const searchField = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    e.preventDefault();
    navigate(`/search?search=${inputSearch}`);
    setInputSearch("");
  };

  const selectedCategory = (e: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
    const liElement = e.currentTarget as HTMLLIElement;
    navigate(`/category?category=${liElement.getAttribute("value")}`);
            ...
            {!cartSlicer.cart.length ? (
              ...
              <div>
                ...
                <div className="d-flex flex-column">
                  {cartSlicer.cart.map((el: Cart, index: number) => {
                    return (
                      <div key={index}>
                        <Link
