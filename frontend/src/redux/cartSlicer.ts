  ...
  reducers: {
    addToCart(state, action) {
      ...
      if (!exist) {
        ...
        state.cart.map(
          (el) => el.id === action.payload.id && (el.quantity = allQuantity)
        );
      }
    },
    ...
    changeQuantity(state, action) {
      state.cart.map(
        (el) =>
          ...
          (el.quantity = action.payload.quantityCart)
      );
    },
...
export const { addToCart, removeProduct, changeQuantity, addTotalPrice } =
  cartSlicer.actions;
