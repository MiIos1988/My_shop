...
import { addProductData, editProductData, getImageUrl, getOneProductData } from "../../service/productService";
...
const CreateProductComponent = () => {
    ...
    useEffect(() => {
        params.id ? (getOneProductData(params.id).then(res => {
    ...
    const editProduct = (e) => {
        e.preventDefault();
        validateInputProduct();
        editProductData(product).then(res => navigate("/dashboard/show-product"))
            .catch(err => console.log(err))
    }
            ...
            <div className="row d-flex align-items-center mt-5">
                ...
                <div className=" col-6 offset-3">
                    <form >
                        ...
                        <input type="number" name="price" className="form-control" placeholder="Price" defaultValue={params.id ? product.price : ""} onChange={handleChange} /><br />
                        <select className="form-select" name="category" aria-label="Default select example" defaultValue={params.id ? product.category : ""} onChange={handleChange}>
                        ...
                        <button className="btn btn-secondary form-control" onClick={params.id ? editProduct : createProduct}>{params.id ? "EDIT PRODUCT" : "ADD PRODUCT"}</button>
                    </form>
                </div>
            </div>
        </div>
